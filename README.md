...
- **AdventureX Hackathon 2025（杭州）Injective 通用赛道第一名**
  - **项目名称**：Injective NFC钱包系统
  - **职责与贡献**：作为团队核心开发者，主导区块链集成与安全模块的设计与实现，集成 Injective 协议进行钱包生成与域名管理，并负责后端 API 开发与部署优化。
  - **主要成果与影响**：
    - 通过 NFC UID 自动生成 Inject 钱包，支持双地址格式（inj 地址 + eth 兼容地址），私钥使用 AES-256-GCM 加密存储，实现便捷的安全钱包注册与重复读取。
    - 开发 .inj 域名系统，支持用户名唯一性验证、编辑与删除，提供个性化身份标识，提升用户体验。
    - 项目在 72 小时黑客松中脱颖而出，获评 Injective 通用赛道第一名，展示了在高性能区块链生态下的创新应用能力，并获得社区认可与潜在集成机会。
    - 该项目强调开源与可扩展性，已开源至 GitHub，便于开发者二次开发，推动 Injective 生态的钱包创新。
  - ![AdventureX Hackathon 获奖照片](images/adventureX.jpg)
